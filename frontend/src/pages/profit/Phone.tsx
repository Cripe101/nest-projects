...
const Phone = () => {
  ...
  const phoneTransac = useQuery({
    queryKey: ["profits", "phone"],
    queryFn: getProfit,
    select: (data) =>
      data.filter(
        (transaction: any) =>
          transaction.description?.toLowerCase() === "phone",
      ),
  });

  // Filter transactions by selected date
  const filteredTransactions = useMemo(() => {
    if (!phoneTransac.data) return [];

    let transactions = [...phoneTransac.data];
    ...
    return transactions;
  }, [phoneTransac.data, selectedMonth]);
          ...
          {paginatedTransactions.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/50 py-10">
              ...
              <p className="mt-1 text-xs text-slate-400">
                Your phone transactions will appear here.
              </p>
            </div>
          )}
        </div>
